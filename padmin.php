...
if ($HTTP_POST_VARS['Action'] == "" or $HTTP_POST_VARS['Action'] == "None"):
  ...
  //Formulate the query
  $sql = "SELECT platformid, osname FROM platform ORDER BY osname ASC";

  //Execute the query and put result in $result
  $result = mysql_query($sql, $db) or die ("Unable to execute query for login: " . mysql_error());

  //Create function box
  echo " <tr><td align=\"center\"><select tabindex=\"0\" name=\"platformid\" size=\"6\">\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Select"):
  ...
  //Formulate queries
  $sql = "SELECT platformid, osname FROM platform WHERE platformid=\"" . $HTTP_POST_VARS["platformid"] . "\"";
...
//Get infro from query
  ...
  echo " <tr><td>" . $pagename . ":</td><td><input type=\"text\" size=\"52\" name=\"osname\" value=\"" . $row["osname"] . "\"></td></tr>\n";
  echo " <tr><td colspan=\"2\" align=\"right\"><input type=\"hidden\" name=\"platformid\" value=\"" . $HTTP_POST_VARS["platformid"] . "\"><input type=\"reset\" value=\"Reset\"><input type=\"submit\" name=\"Action\" value=\"Cancel\"><input type=\"submit\" name=\"Action\" value=\"Update\"></td></tr>\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Update"):
  //Formulate the query
    $sql = "UPDATE platform SET osname = \"" . $HTTP_POST_VARS["osname"] .
        "\" WHERE platformid = \"" . $HTTP_POST_VARS["platformid"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Create"):
  if ($HTTP_POST_VARS["osname"] == ""):
    ...
    //Formulate the query
    $sql = "INSERT INTO platform SET osname=\"" . $HTTP_POST_VARS["osname"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Delete"):
  //Formulate the query
  $sql = "DELETE FROM platform WHERE platformid = \"" . $HTTP_POST_VARS["platformid"] . "\"";
