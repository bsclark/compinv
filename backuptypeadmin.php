...
$pagename = "Backup Type";
$rpage = "./backuptypeadmin.php";
...
if ($HTTP_POST_VARS['Action'] == "" or $HTTP_POST_VARS['Action'] == "None"):
  ...
  //Formulate the query
  $sql = "SELECT backuptypeid, typename FROM backuptype ORDER BY typename ASC";

  //Execute the query and put result in $result
  $result = mysql_query($sql, $db) or die ("Unable to execute query for login: " . mysql_error());

  //Create function box
  echo " <tr><td align=\"center\"><select tabindex=\"0\" name=\"backuptypeid\" size=\"6\">\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Select"):
  ...
  //Formulate queries
  $sql = "SELECT backuptypeid, typename, typenote FROM backuptype WHERE backuptypeid=\"" . $HTTP_POST_VARS["backuptypeid"] . "\"";
...
//Get infro from query
  ...
  echo " <tr><td colspan=\"2\" align=\"right\"><input type=\"hidden\" name=\"backuptypeid\" value=\"" . $HTTP_POST_VARS["backuptypeid"] . "\"><input type=\"reset\" value=\"Reset\"><input type=\"submit\" name=\"Action\" value=\"Cancel\"><input type=\"submit\" name=\"Action\" value=\"Update\"></td></tr>\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Update"):
  //Formulate the query
    $sql = "UPDATE backuptype SET typename = \"" . $HTTP_POST_VARS["typename"] .
        "\", typenote = \"" . $HTTP_POST_VARS["typenote"] .
        "\" WHERE backuptypeid = \"" . $HTTP_POST_VARS["backuptypeid"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Create"):
  if ($HTTP_POST_VARS["typename"] == ""):
    ...
    //Formulate the query
    $sql = "INSERT INTO backuptype SET typename=\"" . $HTTP_POST_VARS["typename"] .
        "\", typenote = \"" . $HTTP_POST_VARS["typenote"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Delete"):
  //Formulate the query
  $sql = "DELETE FROM backuptype WHERE backuptypeid = \"" . $HTTP_POST_VARS["backuptypeid"] . "\"";
