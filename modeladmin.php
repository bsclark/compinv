...
if ($HTTP_POST_VARS['Action'] == "" or $HTTP_POST_VARS['Action'] == "None"):
  ...
  //Formulate the query
  $sql = "SELECT modelid, modelname FROM model ORDER BY modelname ASC";

  //Execute the query and put result in $result
  $result = mysql_query($sql, $db) or die ("Unable to execute query for login: " . mysql_error());

  //Create function box
  echo " <tr><td align=\"center\"><select tabindex=\"0\" name=\"modelid\" size=\"6\">\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Select"):
  ...
  //Formulate queries
  $sql = "SELECT modelid, modelname FROM model WHERE modelid=\"" . $HTTP_POST_VARS["modelid"] . "\"";
...
//Get infro from query
  ...
  echo " <tr><td>" . $pagename . ":</td><td><input type=\"text\" size=\"52\" name=\"modelname\" value=\"" . $row["modelname"] . "\"></td></tr>\n";
  echo " <tr><td colspan=\"2\" align=\"right\"><input type=\"hidden\" name=\"modelid\" value=\"" . $HTTP_POST_VARS["modelid"] . "\"><input type=\"reset\" value=\"Reset\"><input type=\"submit\" name=\"Action\" value=\"Cancel\"><input type=\"submit\" name=\"Action\" value=\"Update\"></td></tr>\n";
...
elseif ($HTTP_POST_VARS["Action"] == "Update"):
  //Formulate the query
    $sql = "UPDATE model SET modelname = \"" . $HTTP_POST_VARS["modelname"] .
        "\" WHERE modelid = \"" . $HTTP_POST_VARS["modelid"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Create"):
  if ($HTTP_POST_VARS["modelname"] == ""):
    ...
    //Formulate the query
    $sql = "INSERT INTO model SET modelname=\"" . $HTTP_POST_VARS["modelname"] . "\"";
...
elseif ($HTTP_POST_VARS["Action"] == "Delete"):
  //Formulate the query
  $sql = "DELETE FROM model WHERE modelid = \"" . $HTTP_POST_VARS["modelid"] . "\"";
